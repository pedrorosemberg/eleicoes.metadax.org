...
| `certidao_criminal` | Release própria: `arquivos_de_certidoes_criminais` — `certidao_criminal_2026_{UF}.zip` (28 arquivos, ~9,5 GB no total) | `scripts/ingest-certidoes.ts` | índice leve em `{ano}/certidoes/{UF}.json` (offset+tamanho); PDFs servidos sob demanda, sem duplicar hospedagem — ver `docs/DATA_SOURCES.md` §1 | `/candidato/[id]` — seção "Certidões criminais", via `/api/certidao/[uf]/[candidato]/[arquivo]` | Ingerido para 22 das 28 UFs — `BA`, `MG`, `PR`, `RJ`, `SC`, `SP` têm o ZIP de origem corrompido no release (falta o índice do ZIP), aguardando reenvio |
