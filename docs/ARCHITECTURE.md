...
| `GET /api/estatisticas-projeto` | GitHub (API pública) + Vercel Web Analytics | 5 min | Estrelas/forks/issues do repositório e visitantes do site — números reais consumidos por `/sobre`. Visitantes fica `null` até o mantenedor habilitar Web Analytics e configurar `VERCEL_API_TOKEN` (fora do alcance deste código, ver `DATA_SOURCES.md` §11). |
